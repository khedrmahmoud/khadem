...
void main() {
  group('InputValidator', () {
    ...
    test('should fail validation when required field is missing', () async {
      final validator = InputValidator({'name': ''}, {'name': 'required'});
    ...
    test(
      'should fail validation when multiple rules are not satisfied',
      () async {
        final validator = InputValidator(
          {'age': 'not-a-number'},
          {'age': 'required|int'},
    ...
    test('should handle multiple fields with multiple rules', () async {
      final validator = InputValidator(
        ...
        {'name': 'required', 'age': 'required|int', 'email': 'required'},
      );
    ...
    test(
      'should skip validation for nullable fields when value is null',
      () async {
        final validator = InputValidator(
          {'name': null, 'age': null},
          {'name': 'nullable|required|string', 'age': 'nullable|int|min:18'},
        );
    ...
    test('should validate non-nullable fields normally', () async {
      final validator = InputValidator(
        {'name': null, 'age': 25},
        {'name': 'required|string', 'age': 'nullable|int|min:18'},
      );
