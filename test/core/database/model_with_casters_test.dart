...
void main() {
  group('Model Integration with Casters', () {
    ...
    test('JsonArrayCast integration - addresses field', () {
      final user = TestUser()
        ..fromJson({
          ...
          'name': 'John Doe',
          'addresses':
              '[{"street":"123 Main","city":"NYC"},{"street":"456 Oak","city":"LA"}]',
        });
    ...
    test('BoolCast integration - isActive field', () {
      // Test with int
      final user1 = TestUser()..fromJson({'id': 1, 'is_active': 1});
      expect(user1.isActive, isTrue);

      // Test with string
      final user2 = TestUser()..fromJson({'id': 2, 'is_active': 'true'});
      expect(user2.isActive, isTrue);

      // Test with bool
      final user3 = TestUser()..fromJson({'id': 3, 'is_active': false});
...
class LegacyModel extends KhademModel<LegacyModel> {
  ...
  @override
  LegacyModel newFactory(Map<String, dynamic> data) =>
      LegacyModel()..fromJson(data);
}
