...
class MockTokenService implements TokenService {
  ...
  Future<List<Map<String, dynamic>>> findTokensByPrefix(String tokenPrefix) async {
...
class TestAuthConfig implements AuthConfig {
  ...
  @override
  List<Map<String, dynamic>> getProvidersForGuard(String guardName) => [getProvider('users')];
...
void main() {
  group('JWT Multi-Device Authentication', () {
    ...
    test('tokens maintain session correlation', () async {
      ...
      final refreshTokenSessionId = getRefreshTokenSessionId(tokens.refreshToken!);
      ...
      final storedToken = await mockTokenService.findToken(tokens.refreshToken!);
    ...
    test('invalidateToken preserves refresh tokens for multi-device sessions', () async {
      ...
      final device1RefreshToken = await mockTokenService.findToken(device1Tokens.refreshToken!);
      expect(device1RefreshToken, isNull);
      
      // Device 2 refresh token should still exist
      final device2RefreshToken = await mockTokenService.findToken(device2Tokens.refreshToken!);
    ...
    test('logoutFromAllDevices invalidates all tokens', () async {
      ...
      expect(await mockTokenService.findToken(device1Tokens.refreshToken!), isNotNull);
      expect(await mockTokenService.findToken(device2Tokens.refreshToken!), isNotNull);
      expect(await mockTokenService.findToken(device3Tokens.refreshToken!), isNotNull);
      ...
      final userTokens = await mockTokenService.findTokensByUser(testUser.getAuthIdentifier(), 'users');
      final refreshTokens = userTokens.where((t) => t['type'] == 'refresh').toList();
      expect(refreshTokens, isEmpty);
    });

    test('invalidateToken with single device logout removes only the correlated refresh token', () async {
      // Generate tokens for multiple devices
      final device1Tokens = await jwtDriver.generateTokens(testUser);
      final device2Tokens = await jwtDriver.generateTokens(testUser);
      ...
      expect(getRefreshTokenSessionId(device1Tokens.refreshToken!), equals(device1SessionId));
      expect(getRefreshTokenSessionId(device2Tokens.refreshToken!), equals(device2SessionId));
