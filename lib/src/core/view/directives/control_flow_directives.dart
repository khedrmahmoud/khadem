...
class SwitchDirective implements ViewDirective {
  static final _switchRegex = RegExp(
    r'@switch\s*\((.*?)\)(.*?)@endswitch',
    dotAll: true,
  );
  static final _caseRegex = RegExp(
    r'@case\s*\((.*?)\)(.*?)@endcase',
    dotAll: true,
  );
  static final _defaultRegex = RegExp(
    r'@default(.*?)@enddefault',
    dotAll: true,
  );
